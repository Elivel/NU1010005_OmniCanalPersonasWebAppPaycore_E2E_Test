...
public class ValidaTicketTransferenciaAPTerCue implements Task {

    private List<Transferencias> transferencias;

    public ValidaTicketTransferenciaAPTerCue(List<Transferencias> transferencias) {
    ...
    public <T extends Actor> void performAs(T actor) {
        ...
        LeerArchivoProperties.esperar2();
        Task.where(actor+" valida que 'Número de cuenta' sea igual a "+"'"+t.getCuentaDestino()+"'", Ensure.that(TICKET_TRANSFERENCIA_AP_NUMCUE_TER.resolveFor(actor).getAttribute(TEXT_CONTENT)).isEqualTo(t.getCuentaDestino())).performAs(actor);
        actor.attemptsTo(Scroll.to(TICKET_TRANSFERENCIA_AP_NUMCUE_TER).andAlignToTop());
        LeerArchivoProperties.esperar2();
    ...
    public static ValidaTicketTransferenciaAPTerCue datosCorrectos(List<Transferencias> transferencias) {
        return Instrumented.instanceOf(ValidaTicketTransferenciaAPTerCue.class).withProperties(transferencias);
    }
}
