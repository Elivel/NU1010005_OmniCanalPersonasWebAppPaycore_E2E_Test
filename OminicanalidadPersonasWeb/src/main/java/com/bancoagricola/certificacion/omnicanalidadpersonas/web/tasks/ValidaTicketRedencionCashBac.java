...
public class ValidaTicketRedencionCashBac implements Task {

    private List<Transferencias> transferencias;

    public ValidaTicketRedencionCashBac(List<Transferencias> transferencias) {
    ...
    public <T extends Actor> void performAs(T actor) {
        ...
        Task.where(actor+" valida que 'Producto a abonar' sea igual a "+"'"+t.getProductoAbonar()+"'",  Ensure.that(PRODUCTOABONAR_TICKET_DON_RED.resolveFor(actor).getAttribute(TEXT_CONTENT)).contains(t.getProductoAbonar())).performAs(actor);
        ...
        Scroll.to(CONCEPTO_TICKET_DON_RED2).performAs(actor);
        LeerArchivoProperties.esperar2();
        Task.where(actor+" valida que 'Concepto' sea igual a "+"'"+t.getConcepto()+"'",  Ensure.that(CONCEPTO_TICKET_DON_RED2.resolveFor(actor).getAttribute(TEXT_CONTENT)).isEqualTo(t.getConcepto())).performAs(actor);
        ...
        Task.where(actor+" valida que 'Fecha aplicada' se muestre", WaitUntil.the(FECHA_APLICADA_TICKET_DON_RED2, isVisible()).forNoMoreThan(1).seconds()).performAs(actor);
    ...
    public static ValidaTicketRedencionCashBac datosCorrectos(List<Transferencias> transferencias) {
        return Instrumented.instanceOf(ValidaTicketRedencionCashBac.class).withProperties(transferencias);
    }
}
