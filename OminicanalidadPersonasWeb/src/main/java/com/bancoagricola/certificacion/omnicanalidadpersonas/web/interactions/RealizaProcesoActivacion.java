...
public class RealizaProcesoActivacion implements Interaction {

    private List<Transferencias> datosTransferencias;

    public RealizaProcesoActivacion(List<Transferencias> datosDCA) {
        this.datosTransferencias = datosDCA;
    }

    @Step("{0} 'realiza proceso de habilitación de permisos'")
    @Override
    public <T extends Actor> void performAs(T actor) {
        ...
        actor.attemptsTo(
                Ensure.that(TTL_SELCC_USUARIO).isDisplayed(),
        ...
        actor.attemptsTo(Click.on(OPC_PERM_PAG.of(t.getPermisos())));
    ...
    public static RealizaProcesoActivacion inicio(List<Transferencias> datosTransferencias) {
        return Instrumented.instanceOf(RealizaProcesoActivacion.class).withProperties(datosTransferencias);
    }
}
