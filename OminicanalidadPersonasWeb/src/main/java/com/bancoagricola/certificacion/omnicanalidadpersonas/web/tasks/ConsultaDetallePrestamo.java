...
import static com.bancoagricola.certificacion.omnicanalidadpersonas.web.userinterface.CuentasPage.ALIAS_EF;
import static com.bancoagricola.certificacion.omnicanalidadpersonas.web.utils.Constantes.TEXT_CONTENT;
import static net.serenitybdd.screenplay.matchers.WebElementStateMatchers.*;

public class ConsultaDetallePrestamo implements Task {
    private List<Transferencias> datosDCA;

    public ConsultaDetallePrestamo(List<Transferencias> datosDCA) {
        this.datosDCA = datosDCA;
    }

    @Step("{0} 'Consulta detalle de préstamo'")
    @Override
    public <T extends Actor> void performAs(T actor) {
        ...
        actor.attemptsTo(
                ...
                Scroll.to(ALIAS_EF.of(ca.getNumeroPrestamo())).andAlignToTop(),
                Ensure.that(ALIAS_EF.of(ca.getNumeroPrestamo())).isDisplayed(),
                Ensure.that(TIPO_CA.of(ca.getNumeroPrestamo())).isDisplayed(),
                Ensure.that(DEUDA_TOTAL_EF.of(ca.getNumeroPrestamo())).isDisplayed(),
                Ensure.that(NUM_EF.of(ca.getNumeroPrestamo())).isDisplayed(),
                Ensure.that(FECHA_PAGO_EF.of(ca.getNumeroPrestamo())).isDisplayed(),
                Ensure.that(ESTADO_EF.of(ca.getNumeroPrestamo())).isDisplayed());
        actor.remember(VariablesSesion.ALIASEF.toString(), ALIAS_EF.of(ca.getNumeroPrestamo()).resolveFor(actor).getAttribute(TEXT_CONTENT));
        actor.remember(VariablesSesion.TIPOEF.toString(), TIPO_CA.of(ca.getNumeroPrestamo()).resolveFor(actor).getAttribute(TEXT_CONTENT));
        actor.remember(VariablesSesion.DEUDATEF.toString(), DEUDA_TOTAL_EF.of(ca.getNumeroPrestamo()).resolveFor(actor).getAttribute(TEXT_CONTENT).replace(" ", "."));
        actor.remember(VariablesSesion.NUMEF.toString(), NUM_EF.of(ca.getNumeroPrestamo()).resolveFor(actor).getAttribute(TEXT_CONTENT));
        actor.remember(VariablesSesion.FECHAPAGEF.toString(), FECHA_PAGO_EF.of(ca.getNumeroPrestamo()).resolveFor(actor).getAttribute(TEXT_CONTENT));
        actor.remember(VariablesSesion.ESTADOEF.toString(), ESTADO_EF.of(ca.getNumeroPrestamo()).resolveFor(actor).getAttribute(TEXT_CONTENT));
        actor.attemptsTo(
                WaitUntil.the(VER_MAS_EXTRAFINANCIAMIENTO.of(ca.getNumeroPrestamo()), isClickable()).forNoMoreThan(60).seconds(),
                Click.on(VER_MAS_EXTRAFINANCIAMIENTO.of(ca.getNumeroPrestamo())),
                WaitUntil.the(TTL_RESUMEN_PRESTAMO, isCurrentlyVisible()).forNoMoreThan(60).seconds(),
                Ensure.that(TTL_RESUMEN_PRESTAMO).isDisplayed(),
        ...
        actor.attemptsTo(
                ...
                WaitUntil.the(TTL_MAS_DETALLE_PRESTAMO, isVisible()).forNoMoreThan(60).seconds(),
                ...
                Ensure.that(TTL_SECC_DAT_GEN).isDisplayed(),
                ...
                Scroll.to(TTL_SECC_CUO_MES).andAlignToTop(),
                Ensure.that(TTL_SECC_CUO_MES).isDisplayed(),
                ...
                Scroll.to(TTL_SECC_CAN_TOT).andAlignToTop(),
                Ensure.that(TTL_SECC_CAN_TOT).isDisplayed(),
                ...
                Ensure.that(TTL_SECC_DET_MOR).isDisplayed(),
    ...
    public static ConsultaDetallePrestamo datosCA(List<Transferencias> datosDCA) {
        return Instrumented.instanceOf(ConsultaDetallePrestamo.class).withProperties(datosDCA);
    }
}