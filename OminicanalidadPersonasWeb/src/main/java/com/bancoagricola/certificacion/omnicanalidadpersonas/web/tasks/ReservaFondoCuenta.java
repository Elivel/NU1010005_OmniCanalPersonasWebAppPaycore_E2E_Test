...
public class ReservaFondoCuenta implements Task {
    private List<Transferencias> informacion;

    public ReservaFondoCuenta(List<Transferencias> informacion) {
        this.informacion = informacion;
    }

    @Step("{0} 'realiza proceso de reserva de fondos'")
    @Override
    public <T extends Actor> void performAs(T actor) {

        actor.attemptsTo(
                Scroll.to(LINK_PAGOES).andAlignToBottom(),
                WaitUntil.the(LINK_PAGOES,isVisible()).forNoMoreThan(30).seconds(),
                WaitUntil.the(LINK_PAGOES,isEnabled()).forNoMoreThan(30).seconds(),
                WaitUntil.the(LINK_PAGOES,isClickable()).forNoMoreThan(30).seconds(),
                Click.on(LINK_PAGOES),
                WaitUntil.the(RESULTADOS.of("P@GOES"), isEnabled()).forNoMoreThan(30).seconds(),
                Scroll.to(BTN_NUEVA_RESERVA).andAlignToBottom(),
                ...
                WaitUntil.the(RESULTADOS.of("Reserva de fondos P@GOES"), isVisible()).forNoMoreThan(30).seconds(),
                WaitUntil.the(RESULTADOS.of("Reserva de fondos P@GOES"), isEnabled()).forNoMoreThan(30).seconds(),
                ...
                WaitUntil.the(RESULTADOS.of("Reservarás fondos P@GOES"),isVisible()).forNoMoreThan(30).seconds(),
        ...
        actor.remember(REFERENCIA_TICKET_VAR2.toString(), RESERVA_CA.resolveFor(actor).getAttribute(TEXT_CONTENT).substring(1).trim());
        System.out.println("Número de reserva: " + actor.recall(REFERENCIA_TICKET_VAR2.toString()).toString());

        /* Comprobante */
        actor.attemptsTo(
                CapturarComprobanteReservaFondo.datosC(informacion),
                Scroll.to(LABEL_COMPROBANTE.of(BOTON_IMPRIMIR)).andAlignToTop());
        /* ------------ */

    }

    public static ReservaFondoCuenta pagoes(List<Transferencias> informacion) {
        return Instrumented.instanceOf(ReservaFondoCuenta.class).withProperties(informacion);
    }
}