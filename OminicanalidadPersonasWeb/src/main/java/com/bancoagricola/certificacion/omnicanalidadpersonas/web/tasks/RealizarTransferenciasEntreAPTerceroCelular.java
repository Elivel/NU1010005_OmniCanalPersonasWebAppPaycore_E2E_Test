...
public class RealizarTransferenciasEntreAPTerceroCelular implements Task {

    private List<Transferencias> datostransferencias;

    public RealizarTransferenciasEntreAPTerceroCelular(List<Transferencias> datostransferencias) {
        this.datostransferencias = datostransferencias;
    }
    @Step("{0} realiza transferencia de 'Ahorro Programado' a 'Tercero' por celular")
    @Override
    public <T extends Actor> void performAs(T actor) {
        ...
        actor.attemptsTo(
                ...
                WaitUntil.the(RADIO_CELULAR, isEnabled()).forNoMoreThan(60).seconds(),
                WaitUntil.the(RADIO_CELULAR, isClickable()).forNoMoreThan(60).seconds(),
                Click.on(RADIO_CELULAR),
                SendKeys.of(t.getCelular()).into(NUM_CUENTA),
    ...
    public static RealizarTransferenciasEntreAPTerceroCelular cuentasTerceros(List<Transferencias> datostransferencias) {
        return Instrumented.instanceOf(RealizarTransferenciasEntreAPTerceroCelular.class).withProperties(datostransferencias);
    }
