...
public class ValidaTicketRecargasPuntosBAFavorito implements Task {

    private List<Transferencias> transferencias;

    public ValidaTicketRecargasPuntosBAFavorito(List<Transferencias> transferencias) {
    ...
    public <T extends Actor> void performAs(T actor) {
        ...
        Scroll.to(DESDE_TICKET_CEL_FAV).andAlignToTop().performAs(actor);
        LeerArchivoProperties.esperar2();
        Task.where(actor+" valida que 'Desde' se muestre", WaitUntil.the(DESDE_TICKET_CEL_FAV, isVisible()).forNoMoreThan(1).seconds()).performAs(actor);
    ...
    public static ValidaTicketRecargasPuntosBAFavorito datosCorrectos(List<Transferencias> transferencias) {
        return Instrumented.instanceOf(ValidaTicketRecargasPuntosBAFavorito.class).withProperties(transferencias);
    }
}
