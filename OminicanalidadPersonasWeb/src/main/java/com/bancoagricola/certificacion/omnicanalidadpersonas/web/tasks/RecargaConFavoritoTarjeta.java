...
public class RecargaConFavoritoTarjeta implements Task {

    private List<Transferencias> datosTransferencias;

    public RecargaConFavoritoTarjeta(List<Transferencias> datosTransferencias) {
    ...
    public <T extends Actor> void performAs(T actor) {
        ...
        actor.attemptsTo(
                ...
                WaitUntil.the(TTL_RECARGA_CELULAR, isVisible()).forNoMoreThan(60).seconds(), WaitUntil.the(SELECC_FAVOR, isClickable()).forNoMoreThan(60).seconds(),
                WaitUntil.the(SELECC_FAVORITO.of(datosTransferencias.get(0).getNombreFavorito()), isEnabled()).forNoMoreThan(60).seconds(),
                WaitUntil.the(SELECC_FAVORITO.of(datosTransferencias.get(0).getNombreFavorito()), isClickable()).forNoMoreThan(60).seconds(),
                Click.on(SELECC_FAVORITO.of(datosTransferencias.get(0).getNombreFavorito())));
        ...
        LeerArchivoProperties.esperar(2);
        actor.attemptsTo(
                SeleccionaMontoPaqueteConFav.datosCA(datosTransferencias.get(0).getMontopaquete()),
        ...
        actor.attemptsTo(
                ValidaTicketRecargasTDCFavorito.datosCorrectos(datosTransferencias));
    }

    public static RecargaConFavoritoTarjeta datosCorrectos(List<Transferencias> datosTransferencias) {
        return Instrumented.instanceOf(RecargaConFavoritoTarjeta.class).withProperties(datosTransferencias);
    }
}
