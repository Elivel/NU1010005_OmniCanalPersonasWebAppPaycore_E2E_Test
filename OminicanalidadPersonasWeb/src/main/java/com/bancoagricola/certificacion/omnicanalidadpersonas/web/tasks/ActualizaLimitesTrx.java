...
public class ActualizaLimitesTrx implements Task {
    private List<Transferencias> datosCAP;

    public ActualizaLimitesTrx(List<Transferencias> datosCAP) {
        this.datosCAP = datosCAP;
    }

    @Step("{0} actualiza los valores de 'Límite por transacción'")
    @Override
    public <T extends Actor> void performAs(T actor) {
        ...
        actor.attemptsTo(
                ...
                WaitUntil.the(BOTON_ACEPTAR_PP, isEnabled()).forNoMoreThan(20).seconds(),
    ...
    public static ActualizaLimitesTrx datosP(List<Transferencias> datosCAP) {
        return Instrumented.instanceOf(ActualizaLimitesTrx.class).withProperties(datosCAP);
    }
}
