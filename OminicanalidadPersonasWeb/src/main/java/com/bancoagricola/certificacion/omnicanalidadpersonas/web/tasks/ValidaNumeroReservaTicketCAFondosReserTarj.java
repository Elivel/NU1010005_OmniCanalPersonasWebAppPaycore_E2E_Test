...
public class ValidaNumeroReservaTicketCAFondosReserTarj implements Task {
    ...
    public <T extends Actor> void performAs(T actor) {
        actor.attemptsTo(
                Scroll.to(LBL_MOVIMIENTOS_TARJETA).andAlignToTop(),
                WaitUntil.the(LBL_MOVIMIENTOS_TARJETA, isEnabled()).forNoMoreThan(15).seconds(),
                WaitUntil.the(OTROS, isEnabled()).forNoMoreThan(15).seconds(),
                WaitUntil.the(OTROS, isClickable()).forNoMoreThan(15).seconds(),
                Click.on(OTROS),
                Scroll.to(LBL_FONDOS_RESERVADOS).andAlignToTop(),
                WaitUntil.the(LBL_FONDOS_RESERVADOS, isEnabled()).forNoMoreThan(15).seconds());
        WaitUntil.the(TABLA_MOV_CA, isVisible()).forNoMoreThan(60).seconds().performAs(actor);

        TABLA_MOVIMIENTOS_REFERENCIA_FON_RES_TARJ.resolveAllFor(actor).forEach(
                elementosLista -> {
                    listaObtenida.clear();
                });
        ...
        Task.where(actor + " valida que se muestre el número de reserva " + "'" + actor.recall(REFERENCIA_TICKET_VAR2.toString()).toString() + "'" + " en la tabla de fondos reservados", Ensure.that(listaObtenida).contains(actor.recall(VariablesSesion.REFERENCIA_TICKET_VAR2.toString()).toString())).performAs(actor);
    }

    public static ValidaNumeroReservaTicketCAFondosReserTarj conLaReferencia() {
        return Instrumented.instanceOf(ValidaNumeroReservaTicketCAFondosReserTarj.class).withProperties();
    }
}