...
public class ValidaTicketSolicElimEstCueEmail_Rec implements Task {

    private List<Transferencias> transferencias;

    public ValidaTicketSolicElimEstCueEmail_Rec(List<Transferencias> transferencias) {
    ...
    public <T extends Actor> void performAs(T actor) {
        ...
        Task.where(actor+" valida que 'Estados de cuenta que empezarás a recibir' sea igual a "+"'"+t.getTarjetaCreditoLista()+"'",  Ensure.that(TICKET_SOLESTACUEEMAIL1.resolveFor(actor).getAttribute(TEXT_CONTENT)).containsIgnoringCase(t.getTarjetaCreditoLista())).performAs(actor);
    ...
    public static ValidaTicketSolicElimEstCueEmail_Rec datosCorrectos(List<Transferencias> transferencias) {
        return Instrumented.instanceOf(ValidaTicketSolicElimEstCueEmail_Rec.class).withProperties(transferencias);
    }
}
