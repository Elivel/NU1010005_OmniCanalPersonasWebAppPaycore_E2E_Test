...
public class FondosReservadosPagoes implements Task {
    private List<Transferencias> datosTransferencias;
    private static List<String> listaObtenida = new ArrayList<>();
    public FondosReservadosPagoes(List<Transferencias> datosFR) {
    ...
    public <T extends Actor> void performAs(T actor) {
        ...
        System.out.println("Referencia: " + actor.recall(VariablesSesion.REFERENCIA_TICKET_VAR.toString()));
        ...
        Task.where(actor+" valida que se muestre el número de referencia "+"'"+actor.recall(VariablesSesion.REFERENCIA_TICKET_VAR.toString()).toString()+"'"+" en la tabla movimientos",  Ensure.that(listaObtenida).contains(actor.recall(VariablesSesion.REFERENCIA_TICKET_VAR.toString()).toString())).performAs(actor);
    }
    public static FondosReservadosPagoes datosFR(List<Transferencias> datosFR) {
        return Instrumented.instanceOf(FondosReservadosPagoes.class).withProperties(datosFR);
    }
}
