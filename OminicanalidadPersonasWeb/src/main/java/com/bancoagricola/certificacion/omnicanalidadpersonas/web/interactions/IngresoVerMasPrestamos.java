...
public class IngresoVerMasPrestamos implements Interaction {

    private List<Transferencias> datosTransferencias;
    public IngresoVerMasPrestamos(List<Transferencias> datosDCA) { this.datosTransferencias = datosDCA;}
    ...
    public <T extends Actor> void performAs(T actor) {
        ...
        actor.attemptsTo(
                ...
                WaitUntil.the(TTL_RESUMEN_PR, isVisible()).forNoMoreThan(60).seconds(),
                WaitUntil.the(TTL_RESUMEN_PR, isEnabled()).forNoMoreThan(60).seconds());
    }

    public static IngresoVerMasPrestamos inicio(List<Transferencias> datosDCA){
        return Instrumented.instanceOf(IngresoVerMasPrestamos.class).withProperties(datosDCA);
    }
}
