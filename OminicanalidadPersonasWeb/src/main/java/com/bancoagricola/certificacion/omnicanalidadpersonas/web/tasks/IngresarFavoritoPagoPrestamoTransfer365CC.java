...
public class IngresarFavoritoPagoPrestamoTransfer365CC implements Task {
    private List<Transferencias> datosDCA;

    public IngresarFavoritoPagoPrestamoTransfer365CC(List<Transferencias> datosDCA) {
    ...
    public <T extends Actor> void performAs(T actor) {
        ...
        /*  */
        actor.attemptsTo(
                IngresoMenuCtaCorrienteOrigenMovimientos.inicio(datosDCA));
        actor.remember(VariablesSesion.MONTO_CUENTA_ORIGEN_FINAL.toString(), OBTENER_SALDO.resolveFor(actor).getText());
    }

    public static IngresarFavoritoPagoPrestamoTransfer365CC inicio(List<Transferencias> datosDCA) {
        return Instrumented.instanceOf(IngresarFavoritoPagoPrestamoTransfer365CC.class).withProperties(datosDCA);
    }
}
