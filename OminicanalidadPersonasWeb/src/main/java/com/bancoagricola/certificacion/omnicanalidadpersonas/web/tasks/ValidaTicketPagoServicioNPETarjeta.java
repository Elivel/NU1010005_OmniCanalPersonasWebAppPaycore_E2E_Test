...
public class ValidaTicketPagoServicioNPETarjeta implements Task {

    private List<Transferencias> transferencias;

    public ValidaTicketPagoServicioNPETarjeta(List<Transferencias> transferencias) {
    ...
    public <T extends Actor> void performAs(T actor) {
        ...
        LeerArchivoProperties.esperar2();
        Task.where(actor+" valida que 'Desde' sea igual a "+"'"+t.getTarjetaCredito()+"'", Ensure.that(TICKET_PAGOSERVICIOSNPE_DESDE.resolveFor(actor).getAttribute(TEXT_CONTENT)).contains(t.getTarjetaCredito())).performAs(actor);
    ...
    public static ValidaTicketPagoServicioNPETarjeta datosCorrectos(List<Transferencias> transferencias) {
        return Instrumented.instanceOf(ValidaTicketPagoServicioNPETarjeta.class).withProperties(transferencias);
    }
}
