...
import static com.bancoagricola.certificacion.omnicanalidadpersonas.web.userinterface.TarjetasPage.VER_MAS_CUENTA_TC;
import static com.bancoagricola.certificacion.omnicanalidadpersonas.web.utils.Constantes.*;
import static net.serenitybdd.screenplay.matchers.WebElementStateMatchers.*;

public class FondosReservadosPagoesTDC implements Task {
    private List<Transferencias> datosTransferencias;
    private static List<String> listaObtenida = new ArrayList<>();
    public FondosReservadosPagoesTDC(List<Transferencias> datosFR) {
    ...
    public <T extends Actor> void performAs(T actor) {
        ...
        Task.where(actor+" valida que se muestre el número de referencia "+"'"+actor.recall(VariablesSesion.REFERENCIA.toString()).toString()+"'"+" en la tabla movimientos",  Ensure.that(listaObtenida).contains(actor.recall(VariablesSesion.REFERENCIA.toString()).toString())).performAs(actor);

    }
    public static FondosReservadosPagoesTDC datosFR(List<Transferencias> datosFR) {
        return Instrumented.instanceOf(FondosReservadosPagoesTDC.class).withProperties(datosFR);
    }
}
