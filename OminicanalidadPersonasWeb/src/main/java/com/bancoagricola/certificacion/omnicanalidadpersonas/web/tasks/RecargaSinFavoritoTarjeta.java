...
public class RecargaSinFavoritoTarjeta implements Task {

    private List<Transferencias> datosTransferencias;

    public RecargaSinFavoritoTarjeta(List<Transferencias> datosTransferencias) {
    ...
    public <T extends Actor> void performAs(T actor) {
        ...
        actor.attemptsTo(
                SendKeys.of(datosTransferencias.get(0).getCelular()).into(TXT_CELULAR));
        LeerArchivoProperties.esperar(2);
        actor.attemptsTo(
                WaitUntil.the(OPC_COMPAÑIA, isCurrentlyEnabled()).forNoMoreThan(60).seconds(),
                ...
                SeleccionarCompania.datosCA(datosTransferencias.get(0).getCompania()));
        LeerArchivoProperties.esperar(3);
        actor.attemptsTo(
                WaitUntil.the(OPC_PAQ_MONTO, isCurrentlyEnabled()).forNoMoreThan(60).seconds(),
        ...
        LeerArchivoProperties.esperar(2);
        actor.attemptsTo(
                SeleccionaMontoPaqueteSinFav.datosCA(datosTransferencias.get(0).getMontopaquete()),
                Scroll.to(TXT_CONCEPTO).andAlignToTop(),
                SendKeys.of(datosTransferencias.get(0).getConcepto()).into(TXT_CONCEPTO),
        ...
        actor.attemptsTo(
                AgregaFavorito.datosCA(datosTransferencias.get(0).getNombreFavorito()),
                ValidaTicketRecargasTDC.datosCorrectos(datosTransferencias));
    }

    public static RecargaSinFavoritoTarjeta datosCorrectos(List<Transferencias> datosTransferencias) {
        return Instrumented.instanceOf(RecargaSinFavoritoTarjeta.class).withProperties(datosTransferencias);
    }
}
